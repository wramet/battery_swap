...
class JbdBms(SerialConnection):
    ...
    async def connect(self):
        if await super().connect() and self.serial.is_open:
            self.logger.info(f"Trying to connect {self.port} ...")
            #await self.write_and_read('dda50300fffd77')
            return self.logger.info(f"Connected to {self.port} successfully")
        self.logger.error("Bad Connection")

    @backoff.on_predicate(backoff.expo, is_empty_bytearray, max_tries=5, max_time=60,on_giveup=lambda details: JbdBms.on_giveup(details), logger=logger)
    async def write_and_read(self, command_hex):
        await self.serial.write_async(command_hex)
        data = await self.serial.read_async(50)
        return data
    
    @classmethod
    def on_giveup(cls, details):
        cls.logger.error(f"Failed to connect after {details['tries']} tries. Exiting.")
        sys.exit(1)
    ...
    async def control_fan(self,fan_position : int, state:str):
        ...
        module_address = 0x01 + (fan_position - 1) // 4  # พัดลม 4 ตัวต่อโมดูล
        relay_number = (fan_position - 1) % 4
        function_code = 0x05
